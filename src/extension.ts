...
export function activate(context: vscode.ExtensionContext) {
  ...
  const condaManager = new CondaManager(outputChannel);
  const envTreeProvider = new CondaEnvTreeProvider();
  envTreeProvider.setCondaManager(condaManager);
  const quickActionsProvider = new CondaQuickActionsProvider();
  const remoteAdapter = new RemoteAdapter(outputChannel);
  ...
  registerCommands(context, condaManager, envTreeProvider, quickActionsProvider, outputChannel, remoteAdapter);
    ...
    vscode.window.withProgress({
      ...
      try {
        ...
        if (healthOnStartup && info) {
          ...
          const errors = result.checks.filter((c: { status: string }) => c.status === 'error');
          const warnings = result.checks.filter((c: { status: string }) => c.status === 'warning');
          if (errors.length > 0) {
            vscode.window.showWarningMessage(

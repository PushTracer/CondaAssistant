...
class CondaManager {
    ...
    async getCondaInfo() {
        try {
            const versionOutput = await (0, utils_1.execConda)(['--version']);
            const condaVersion = versionOutput.replace('conda ', '').trim();
            const envListOutput = await (0, utils_1.execConda)(['env', 'list']);
            const envs = (0, utils_1.parseCondaEnvList)(envListOutput);
            ...
            return {
                ...
                envs,
                defaultEnv,
                condaPath: (0, utils_1.getCondaPath)()
            };
        }
        catch (err) {
    ...
    async enrichAllEnvironments(envs, onProgress) {
        const result = [];
        for (let i = 0; i < envs.length; i++) {
            result.push(await this.enrichEnvironment(envs[i]));
            if (onProgress)
                onProgress(i + 1, envs.length);
        }
        return result;
    }
    async getPythonVersion(envName) {
        try {
            const pyPath = process.platform === 'win32'
                ? path.join((0, utils_1.getEnvPath)(envName), 'python.exe')
                : path.join((0, utils_1.getEnvPath)(envName), 'bin', 'python');
            if (fs.existsSync(pyPath)) {
                const output = await (0, utils_1.execCommand)(`"${pyPath}" --version`, 10000);
    ...
    async getPackageCount(envName) {
        try {
            const output = await (0, utils_1.execConda)(['list', '-n', envName, '--json']);
        ...
        try {
            ...
            if (envName === 'base' || envName === rootPrefix) {
                if (fs.existsSync(rootPrefix)) {
                    return (0, utils_1.formatBytes)(await this.getDirSize(rootPrefix));
                }
                return '未知';
            }
            const envDirs = info.envs_dirs || [];
            for (const dir of envDirs) {
                const envPath = path.join(dir, envName);
                if (fs.existsSync(envPath)) {
                    return (0, utils_1.formatBytes)(await this.getDirSize(envPath));
                }
            }
            const fallback = path.join(rootPrefix, 'envs', envName);
            if (fs.existsSync(fallback)) {
                return (0, utils_1.formatBytes)(await this.getDirSize(fallback));
            }
            return '未知';
        }
        catch (err) {
    ...
    async deleteEnvironment(name) {
        try {
            await (0, utils_1.execConda)(['remove', '-y', '-n', name, '--all']);
            this.outputChannel.appendLine(`环境 ${name} 已删除`);
            const cleanChoice = await vscode.window.showInformationMessage(`环境 ${name} 已删除。是否同时清理 Conda 包缓存以释放磁盘空间？`, '清理缓存', '暂不清理');
            if (cleanChoice === '清理缓存') {
                await (0, utils_1.execConda)(['clean', '-afy'], 120000);
    ...
    async cloneEnvironment(src, dst) {
        try {
            await (0, utils_1.execConda)(['create', '-y', '-n', dst, '--clone', src], 180000);
    ...
    async exportEnvironment(name, format = 'yml') {
        try {
            ...
            const output = await (0, utils_1.execConda)(['env', flag, '-n', name]);
    ...
    async importEnvironment() {
        try {
            ...
            if (ext === '.txt') {
                await (0, utils_1.execConda)(['install', '-y', '--file', filePath], 180000);
            }
            else {
                await (0, utils_1.execConda)(['env', 'create', '-f', filePath], 180000);
            }
    ...
    async installPackage(envName, pkgName) {
        try {
            await (0, utils_1.execConda)(['install', '-y', '-n', envName, pkgName], 120000);
    ...
    async uninstallPackage(envName, pkgName) {
        try {
            await (0, utils_1.execConda)(['remove', '-y', '-n', envName, pkgName]);
    ...
    async listPackages(envName) {
        try {
            return await (0, utils_1.execConda)(['list', '-n', envName]);
        }
        catch {
            return '';
        }
    }
    async listPackagesJSON(envName) {
        try {
            const output = await (0, utils_1.execConda)(['list', '-n', envName, '--json']);
            const packages = JSON.parse(output);
    ...
    async listPackagesWithSize(envName) {
        try {
            ...
            if (envName === 'base') {
                ...
                if (!envPath)
                    envPath = path.join(rootPrefix, 'envs', envName);
            }
            const condaMeta = path.join(envPath, 'conda-meta');
            if (!fs.existsSync(condaMeta)) {
                return await (0, utils_1.execConda)(['list', '-n', envName]);
            }
            const files = fs.readdirSync(condaMeta).filter((f) => f.endsWith('.json') && !f.endsWith('.xz'));
            const pkgSizes = [];
            ...
            const realSize = fs.existsSync(envPath) ? (0, utils_1.formatBytes)(await this.getDirSize(envPath)) : '?';
            let output = `# 包分析 (${pkgSizes.length} 个包)\n`;
            output += `包净体积: ${(0, utils_1.formatBytes)(total)}  |  环境目录实际大小: ${realSize}\n\n`;
            output += `${'包名'.padEnd(30)} ${'版本'.padEnd(18)} ${'大小'.padEnd(10)}\n`;
            output += `${'─'.repeat(58)}\n`;
            for (const pkg of pkgSizes) {
                output += `${pkg.name.padEnd(30)} ${pkg.version.padEnd(18)} ${(0, utils_1.formatBytes)(pkg.size).padStart(10)}\n`;
            }
            output += `${'─'.repeat(58)}\n`;
            output += `${'包净体积总计'.padEnd(30)} ${''.padEnd(18)} ${(0, utils_1.formatBytes)(total).padStart(10)}\n`;
    ...
    async getPackageDeps(envName, pkgName) {
        try {
            const output = await (0, utils_1.execConda)(['list', '-n', envName, '--json']);
            const packages = JSON.parse(output);
            const pkg = packages.find((p) => p.name === pkgName);
            if (pkg && pkg.depends) {
                return pkg.depends.map((d) => d.split(' ')[0]);
            }
            return [];
        }
        catch {
    ...
    async activateEnvironment(envName) {
        try {
            ...
            terminal.show();
            const condaPath = (0, utils_1.getCondaPath)();
            ...
            ensureCondaDirs((0, utils_1.getEnvPath)(envName)); // target env
            ensureCondaDirs((0, utils_1.getCondaPrefix)()); // base env (for deactivation)
